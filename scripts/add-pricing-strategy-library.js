...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({ id: `ps_${num}_${tag.replace(/-/g, "_")}`, title, description, prompt: promptText, category_id: "business", subcategory: TAG_MAP[tag] || "Pricing Foundations", tags: [tag, "pricing", "strategy", "business"], model: "ChatGPT / Claude", source: "curated" });
  }
  return prompts;
}
async function main() {
  const prompts = parsePrompts(fs.readFileSync("/Users/mohammedbarbar/Downloads/pricing-strategy-prompt-library.md", "utf8"));
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "business");
  console.log(`\n✅ ${inserted} inserted, Business total: ${count}`);
}
