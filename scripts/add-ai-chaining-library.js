...
const TAG_MAP = {
  "foundation":  "AI Chaining Foundations",
...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({ id: `aic_${num}_${tag.replace(/-/g, "_")}`, title, description, prompt: promptText, category_id: "aiworkflows", subcategory: TAG_MAP[tag] || "AI Chaining Foundations", tags: [tag, "ai agents", "prompt chaining", "automation"], model: "Claude / ChatGPT", source: "curated" });
  }
  return prompts;
}
async function main() {
  const prompts = parsePrompts(fs.readFileSync("/Users/mohammedbarbar/Downloads/ai-prompt-chaining-agents-prompt-library.md", "utf8"));
  ...
  console.log(`\n✅ ${inserted} inserted, AI Workflows total: ${count}`);
}
