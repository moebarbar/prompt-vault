...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `hr_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
      category_id: "hr",
      subcategory: TAG_MAP[tag] || "Utility",
      tags: [tag, "hr", "human resources"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/hr-prompt-library.md", "utf8");
  const prompts = parsePrompts(markdown);

  console.log(`\n👥 Parsed ${prompts.length} prompts from hr-prompt-library.md\n`);
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "hr");
  console.log(`\n✅ Done — inserted ${inserted} prompts`);
  console.log(`   HR category total: ${count}`);
}
