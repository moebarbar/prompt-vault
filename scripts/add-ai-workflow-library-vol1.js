...
function parseWorkflows(markdown) {
  const prompts = [];
  // Split on WORKFLOW headers (different from other libraries)
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/ai-workflow-library-vol1.md", "utf8");
  const prompts = parseWorkflows(markdown);

  console.log(`\n🤖 Parsed ${prompts.length} workflows from ai-workflow-library-vol1.md\n`);
