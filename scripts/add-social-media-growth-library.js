...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `smg_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
      category_id: "social",
      subcategory: TAG_MAP[tag] || "Growth Strategy",
      tags: [tag, "social media", "growth", "content"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/social-media-growth-prompt-library.md", "utf8");
  const prompts = parsePrompts(markdown);

  console.log(`\n📱 Parsed ${prompts.length} prompts from social-media-growth-prompt-library.md\n`);
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "social");
  console.log(`\n✅ Done — inserted ${inserted} prompts`);
  console.log(`   Social Media category total: ${count}`);
}
