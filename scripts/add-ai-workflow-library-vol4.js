...
function parseWorkflows(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `aiw4_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/ai-workflow-library-vol4.md", "utf8");
  const prompts = parseWorkflows(markdown);

  console.log(`\n🤖 Parsed ${prompts.length} workflows from ai-workflow-library-vol4.md\n`);
