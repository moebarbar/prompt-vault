...
const TAG_MAP = {
  "strategy":        "List Building Strategy",
  "landing-page":    "Landing Pages & Opt-ins",
  "welcome-sequence":"Welcome Sequences",
  "growth":          "List Growth Tactics",
  "monetization":    "List Monetization",
  "list-health":     "List Health & Deliverability",
};
...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `el_${num}_${tag.replace(/-/g, "_")}`,
      title, description, prompt: promptText,
      category_id: "emaillist",
      subcategory: TAG_MAP[tag] || "List Building Strategy",
      tags: [tag, "email list", "newsletter", "lead generation"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/email-list-building-prompt-library.md", "utf8");
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "emaillist");
  console.log(`\n✅ Done — ${inserted} inserted, Email List total: ${count}`);
}
