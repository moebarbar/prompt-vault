...
const TAG_MAP = {
  ...
  "gtm":           "Go-to-Market Research",
  "analysis":      "Data & Analysis",
  "intelligence":  "Market Intelligence",
  "saas":          "SaaS Research",
  "local":         "Local Market Research",
...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `mr_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
      category_id: "marketresearch",
      subcategory: TAG_MAP[tag] || "Research Foundations",
      tags: [tag, "market research", "research"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/market-research-prompt-library.md", "utf8");
  const prompts = parsePrompts(markdown);

  console.log(`\n🔭 Parsed ${prompts.length} prompts from market-research-prompt-library.md\n`);
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "marketresearch");
  console.log(`\n✅ Done — inserted ${inserted} prompts`);
  console.log(`   Market Research category total: ${count}`);
}
