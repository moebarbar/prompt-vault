...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `img_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
      category_id: "imagegen",
      subcategory: TAG_MAP[tag] || "Image Gen Foundations",
      tags: [tag, "image generation", "midjourney", "dall-e"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/image-generation-prompt-library.md", "utf8");
  const prompts = parsePrompts(markdown);

  console.log(`\n🎨 Parsed ${prompts.length} prompts from image-generation-prompt-library.md\n`);
