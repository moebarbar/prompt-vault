...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `seo_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
      category_id: "seo",
      subcategory: TAG_MAP[tag] || "SEO Strategy",
      tags: [tag, "seo"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/seo-prompt-library.md", "utf8");
  const prompts = parsePrompts(markdown);

  console.log(`\n🔍 Parsed ${prompts.length} prompts from seo-prompt-library.md\n`);
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "seo");
  console.log(`\n✅ Done — inserted ${inserted} prompts`);
  console.log(`   SEO category total: ${count}`);
}
