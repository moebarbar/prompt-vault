...
const TAG_MAP = {
  "foundation":         "Writing Foundations",
  "blog":               "Blog & Articles",
  "copy":               "Copywriting",
  "business":           "Business Writing",
  "social":             "Social Writing",
  "fiction":            "Fiction",
  "creative-nonfiction":"Creative Nonfiction",
  "poetry":             "Poetry",
  "screenplay":         "Screenplay & Scripts",
  "speech":             "Speeches & Presentations",
  "academic":           "Academic Writing",
  "specialized":        "Specialized Writing",
  "tools":              "Writing Tools & Productivity",
};
...
function parsePrompts(markdown) {
  ...
  for (const block of blocks) {
    ...
    prompts.push({
      id: `wrt_${num}_${tag.replace(/-/g, "_")}`,
      title,
      description,
      prompt: promptText,
      category_id: "writing",
      subcategory: TAG_MAP[tag] || "Writing Foundations",
      tags: [tag, "writing"],
...
async function main() {
  const markdown = fs.readFileSync("/Users/mohammedbarbar/Downloads/writing-prompt-library.md", "utf8");
  const prompts = parsePrompts(markdown);

  console.log(`\n✍️  Parsed ${prompts.length} prompts from writing-prompt-library.md\n`);
  ...
  const { count } = await supabase.from("prompts").select("*", { count: "exact", head: true }).eq("category_id", "writing");
  console.log(`\n✅ Done — inserted ${inserted} prompts`);
  console.log(`   Writing category total: ${count}`);
}
